...
void search_collision_in_bvh(
    ...
    if (bvhnodes[i_bvhnode].is_leaf()) { // this is leaf node
        ...
        // do something
        auto res = ray_triangle_intersection(ray_org, ray_dir, i_tri, tri2vtx, vtx2xyz);
        if (res != std::nullopt) {
            is_hit = true;
            hit_pos = res->first;
            hit_normal = res->second;
            const float depth = (hit_pos - ray_org).dot(ray_dir);
            hit_depth = depth;
        }
            ...
            if (res) { // ambient occlusion computation
                ...
                for (unsigned int i_sample = 0; i_sample < num_sample_ao; ++i_sample) {
                    ...
                    if (!res1) { // if the ray does not hit anything
                        float tmp = 1.0f*dir.dot(nrm)/pdf/float(M_PI); // Problem 3: This is a bug. write some correct code (hint: use `dir.dot(nrm)`, `pdf`, `M_PI`).
                        sum += tmp;
                    }
                }
                sum = sum > num_sample_ao ? num_sample_ao : sum;
